...
This bot watches [Stock Market Genius]((https://twitter.com/stockmktgenius))
...
the relevant stocks according to the expected market reaction. It can also tweets
out a summary of its findings in real time if a bot account is set.
...
notified whenever Stock Market Genius. The entity detection and sentiment analysis is
done using Google's
...
handled and starts streaming Stock Market Genius's feed:
...
```shell
docker build -t twitter-trade-bot .
docker tag twitter-trade-bot gcr.io/<YOUR_GCP_PROJECT_NAME>/twitter-trade-bot
docker push gcr.io/<YOUR_GCP_PROJECT_NAME>/twitter-trade-bot:latest
```
